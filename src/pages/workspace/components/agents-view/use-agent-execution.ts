...
export function useAgentExecution({ taskId }: UseAgentExecutionProps) {
      ...
      if (message.toolInvocations && message.toolInvocations.length > 0) {
        for (const invocation of message.toolInvocations) {
          ...
          if (invocation.state === 'result' && 'result' in invocation) {
            if (invocation.toolName === 'agentTaskComplete' && invocation.result) {
              const result = invocation.result as { status: 'review' | 'need_clarification'; message: string };
              completeAgentTask(currentProject, taskId, result.status, result.message);
            } else if (invocation.toolName === 'agentRequestClarification' && invocation.result) {
              const result = invocation.result as { questions: string[]; context?: string };
              const message = `Clarification needed:\n${result.questions.map((q: string, i: number) => `${i + 1}. ${q}`).join('\n')}${result.context ? `\n\nContext: ${result.context}` : ''}`;
              completeAgentTask(currentProject, taskId, 'need_clarification', message);
            }
          }
        }
      } else {
  ...
  }, [currentProject, taskId, currentTask, initialMessages, addMessage, updateAgentStep, setMessages, reload]);
