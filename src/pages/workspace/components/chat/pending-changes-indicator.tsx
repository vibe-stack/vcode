...
export function PendingChangesIndicator({
  ...
  const pendingData = useMemo(() => {
    ...
    const pendingMessagesCount = new Set(
      pendingSnapshots.map((s) => s.messageId),
    ).size;
  ...
  return (
    <div className="flex items-center gap-2 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
      <AlertTriangle className="h-3 w-3" />
      <span>
        {pendingData.count} pending file change
        {pendingData.count !== 1 ? "s" : ""}
        {pendingData.messagesCount > 1 &&
          ` across ${pendingData.messagesCount} messages`}
      </span>
    </div>
  );
}
