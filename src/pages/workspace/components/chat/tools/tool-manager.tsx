...
export function ToolManager({ onClose }: ToolManagerProps) {
  ...
  const handleToggleTool = (toolName: string) => {
    ...
    // Update the registry
    toolRegistry.setToolEnabled(
      toolName,
      !toolRegistry.isToolEnabled(toolName),
    );
  };

  const groupedTools = Object.values(configs).reduce(
    (acc, config) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure which tools are available to Grok
          </p>
        </div>
        {onClose && (
        ...
        <Card key={category}>
          <CardHeader>
            ...
            <CardDescription>
              ...
              {category === "directory" &&
                "Tools for managing directories and folder structures"}
              {category === "search" &&
                "Tools for searching and finding content"}
              {category === "project" &&
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <div className="text-center">
              ...
              <div className="text-muted-foreground text-sm">Enabled Tools</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-600">
                {
                  Object.values(configs).filter((c) => c.requiresConfirmation)
                    .length
                }
              </div>
              <div className="text-muted-foreground text-sm">
