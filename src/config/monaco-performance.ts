...
class MonacoPerformanceMonitor {
  ...
  private recordMetrics(editorId: string, metrics: PerformanceMetrics): void {
    ...
    editorData.totalMemory = editorData.metrics.reduce(
      (sum, m) => sum + m.memoryUsage,
      0,
    );
    editorData.averageLoadTime =
      editorData.metrics.reduce((sum, m) => sum + m.loadTime, 0) /
      editorData.metrics.length;
  ...
  private checkPerformanceThresholds(
    ...
    if (loadTime > this.loadTimeThreshold) {
      warnings.push(
        `Slow loading time: ${loadTime.toFixed(2)}ms (threshold: ${this.loadTimeThreshold}ms)`,
      );
    }

    if (memoryUsage > this.memoryThreshold) {
      warnings.push(
        `High memory usage: ${(memoryUsage / 1024 / 1024).toFixed(2)}MB (threshold: ${this.memoryThreshold / 1024 / 1024}MB)`,
      );
    }

    if (warnings.length > 0) {
      console.warn(
        `Monaco Editor Performance Warning [${editorId}]:`,
        warnings,
      );
    }
  }
  ...
  getOverallPerformanceSummary(): {
    ...
    const totalMemory = editors.reduce(
      (sum, editor) => sum + editor.totalMemory,
      0,
    );
    const averageLoadTime =
      editors.reduce((sum, editor) => sum + editor.averageLoadTime, 0) /
      editors.length;
  ...
  getPerformanceRecommendations(): string[] {
    ...
    if (summary.totalMemory > 200 * 1024 * 1024) {
      // 200MB
      recommendations.push(
        "Consider reducing the number of open editors or file sizes",
      );
    }
    ...
    const editors = Array.from(this.performanceData.values());
    const largeFileEditors = editors.filter(
      (editor) =>
    ...
    if (largeFileEditors.length > 0) {
      recommendations.push(
        "Large files detected - consider using read-only mode or syntax highlighting optimization",
      );
    }
...
export function optimizeEditorForLargeFiles(
  fileSize: number,
): Partial<
