...
export function addAgentEventListeners() {
  // Agent messaging
  ipcMain.handle(AGENT_SEND_MESSAGE_CHANNEL, async (event, { taskId, messages, requestId }) => {
    try {
      ...
      const processStream = async () => {
        try {
          while (true) {
            ...
            if (done) {
              webContents.send(AGENT_STREAM_END_CHANNEL, { taskId, requestId });
              break;
            }
  ...
  ipcMain.handle(AGENT_CREATE_WORKTREE_CHANNEL, async (event, { taskId, branchName }) => {
    try {
