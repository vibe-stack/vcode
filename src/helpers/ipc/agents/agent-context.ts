...
export function exposeAgentContext() {
  ...
  contextBridge.exposeInMainWorld("agents", {
    ...
    onStreamChunk: (
      ...
      ipcRenderer.on(AGENT_STREAM_CHUNK_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
    ...
    onStreamEnd: (
      ...
      ipcRenderer.on(AGENT_STREAM_END_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
    ...
    onStreamError: (
      ...
      ipcRenderer.on(AGENT_STREAM_ERROR_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
    ...
    startAgent: (taskId: string) =>
      ipcRenderer.invoke(AGENT_START_CHANNEL, { taskId }),
    stopAgent: (taskId: string) =>
      ipcRenderer.invoke(AGENT_STOP_CHANNEL, { taskId }),
    pauseAgent: (taskId: string) =>
      ipcRenderer.invoke(AGENT_PAUSE_CHANNEL, { taskId }),
    resumeAgent: (taskId: string) =>
      ipcRenderer.invoke(AGENT_RESUME_CHANNEL, { taskId }),

    onStatusUpdate: (callback: (data: AgentStatusUpdate) => void) => {
      ipcRenderer.on(AGENT_STATUS_UPDATE_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
    ...
    onWorktreeStatusChange: (callback: (data: AgentWorktreeInfo) => void) => {
      ipcRenderer.on(AGENT_WORKTREE_STATUS_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
