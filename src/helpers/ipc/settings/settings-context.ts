...
export function exposeSettingsContext() {
  ...
  contextBridge.exposeInMainWorld("settingsApi", {
    ...
    set: (key: string, value: any) =>
      ipcRenderer.invoke(SETTINGS_SET_CHANNEL, key, value),
    ...
    import: (settingsJson: string) =>
      ipcRenderer.invoke(SETTINGS_IMPORT_CHANNEL, settingsJson),

    // Secure settings (API keys, tokens, etc.)
    getSecure: (key: string) =>
      ipcRenderer.invoke(SETTINGS_GET_SECURE_CHANNEL, key),
    setSecure: (key: string, value: string) =>
      ipcRenderer.invoke(SETTINGS_SET_SECURE_CHANNEL, key, value),
    deleteSecure: (key: string) =>
      ipcRenderer.invoke(SETTINGS_DELETE_SECURE_CHANNEL, key),
