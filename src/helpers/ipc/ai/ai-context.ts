...
import {
  AI_SEND_MESSAGE_CHANNEL,
  AI_STREAM_CHUNK_CHANNEL,
  AI_STREAM_END_CHANNEL,
...
export function exposeAIContext() {
  ...
  contextBridge.exposeInMainWorld("ai", {
    ...
    onStreamChunk: (
      ...
      ipcRenderer.on(AI_STREAM_CHUNK_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },

    onStreamEnd: (callback: (data: { requestId: string }) => void) => {
      ipcRenderer.on(AI_STREAM_END_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
    ...
    onStreamError: (
      ...
      ipcRenderer.on(AI_STREAM_ERROR_CHANNEL, (_event: any, data: any) =>
        callback(data),
      );
    },
